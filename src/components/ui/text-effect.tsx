...
import { AnimatePresence, motion, type TargetAndTransition, type Transition, type Variants } from "framer-motion";
import React from "react";

/**
 * Text reveal primitive (motion-primitives / ibelick "text-effect").
 *
 * Splits its children into characters, words or lines and staggers each piece
 * in. Respects reduced-motion through framer-motion's own handling of the
 * `prefers-reduced-motion` media query in the consuming app.
 */
...
// Compatibility fix for framer-motion 12: `Variants[key]` widens to `Variant`,
// which includes `TargetResolver` (a function). Narrow from `unknown` so the
// guard accepts a raw variant entry as well as a resolved target.
function hasTransition(variant?: unknown): variant is TargetAndTransition & { transition?: Transition } {
